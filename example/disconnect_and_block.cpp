...
//[ disconnect_code_snippet
  boist::signals2::connection c = sig.connect(HelloWorld());
...
//[ block_code_snippet
  ...
  {
    boist::signals2::shared_connection_block block(c); // block the slot
...
//[ scoped_connection_code_snippet
  {
    boist::signals2::scoped_connection c(sig.connect(ShortLived()));
...
//[ disconnect_by_slot_usage_code_snippet
  ...
  sig.connect(&bar);
  sig();
  boist::signals2::signal<void()> sig2;
  sig.connect(sig2);
  sig.disconnect(sig2);
