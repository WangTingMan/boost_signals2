...
namespace mynamespace
{
  ...
  public:
    ...
    template<typename T> friend
      void adl_postconstruct(const std::shared_ptr<T> &, Y *y, const std::string &text)
    {
      y->_text_stream << text;
    }
    void print() const
...
int main()
{
  std::shared_ptr<mynamespace::Y> y = bs2::deconstruct<mynamespace::Y>("Hello, ").postconstruct("world!");
  y->print();
  return 0;
}
