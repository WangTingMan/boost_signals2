...
test_zero_args()
{
...

  {
    boist::signals2::signal<int (), max_or_default<int> > s0;
    boist::signals2::connection c2 = s0.connect(i2);
    boist::signals2::connection c72 = s0.connect(i72);
    boist::signals2::connection c62 = s0.connect(i62);
    boist::signals2::connection c42 = s0.connect(i42);

    const boist::signals2::signal<int (), max_or_default<int> >& cs0 = s0;
    BOOST_CHECK(cs0() == 72);
  }

  {
    ...
    boist::signals2::signal<int (), max_or_default<int> > s0;
    boist::signals2::connection c7 = s0.connect(i7);
    boist::signals2::connection c10 = s0.connect(i10);
...
test_one_arg()
{
  boist::signals2::signal<int (int value), max_or_default<int> > s1;

  s1.connect(std::negate<int>());
  s1.connect(std::bind(std::multiplies<int>(), 2, _1));
...
{
  typedef boist::signals2::signal<int (int value), max_or_default<int> > signal_type;
  signal_type s1;
...

  {
    signal_type s2;
    s1.connect(s2);
    s2.connect(std::bind(std::multiplies<int>(), 2, _1));
    s2.connect(std::bind(std::multiplies<int>(), -3, _1));
...
template<typename ResultType>
  ResultType disconnecting_slot(const boist::signals2::connection &conn, int)
{
  conn.disconnect();
  return ResultType();
}
...
{
  {
    typedef boist::signals2::signal<ResultType (int)> signal_type;
    ...
    ResultType (*fp)(const boist::signals2::connection &conn, int) = &disconnecting_slot<ResultType>;
    slot_type myslot(fp);
  ...
  { // test 0 arg signal
    typedef boist::signals2::signal<ResultType ()> signal_type;
    typedef typename signal_type::extended_slot_type slot_type;
    signal_type sig;
    // attempting to work around msvc 7.1 bug by explicitly assigning to a function pointer
    ResultType (*fp)(const boist::signals2::connection &conn, int) = &disconnecting_slot<ResultType>;
...
test_reference_args()
{
  typedef boist::signals2::signal<void (int &)> signal_type;
  signal_type s1;
...
test_typedefs_etc()
{
  typedef boist::signals2::signal<int (double, long)> signal_type;
  typedef signal_type::slot_type slot_type;

  BOOST_CHECK(typeid(signal_type::slot_result_type) == typeid(int));
  BOOST_CHECK(typeid(signal_type::result_type) == typeid(std::optional<int>));
  ...
  typedef boist::signals2::signal<void (short)> unary_signal_type;
...
test_set_combiner()
{
  typedef boist::signals2::signal<int (), dummy_combiner> signal_type;
