...
typedef boist::signals2::signal<void ()> sig0_type;
...
{
  typedef boist::signals2::signal<void (), slot_counter> signal_type;
  signal_type sig;
  {
    boist::signals2::scoped_connection conn(sig.connect(&my_slot));
    BOOST_CHECK(sig() == 1);
...
{
  boist::signals2::signal<int& ()> rTest;
  ref_returner rr;
...
{
  boist::signals2::signal<std::unique_ptr<int>()> sig;
