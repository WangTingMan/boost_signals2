...
#include <boost_signals2/shared_connection_block.hpp>
...
static boost::array<boist::signals2::connection, 4> connections;
...
BOOST_AUTO_TEST_CASE(test_main)
{
  ...
  {
    // Blocking 2
    boist::signals2::shared_connection_block block(connections.at(2));
...

  {
    ...
    const boist::signals2::connection conn = connections.at(1);
    boist::signals2::shared_connection_block block(conn);
    test_output.str("");
    s0();
  ...
  {
    // initially unblocked
    boist::signals2::shared_connection_block block(connections.at(3), false);
    BOOST_CHECK(block.blocking() == false);
    test_output.str("");
    s0();
    BOOST_CHECK(test_output.str() == "0123");
...

  {
    ...
    boist::signals2::shared_connection_block block;
    ...
    {
      block.unblock();
      boist::signals2::shared_connection_block block2(connections.at(0));
